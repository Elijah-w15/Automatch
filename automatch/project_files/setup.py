    ...
    try:
        if os.name == "nt":      # native Windows: a .bat on the default PATH
            ...
            (bindir / "automatch.bat").write_text(
                "@echo off\r\n"
                f'cd /d "{HERE}"\r\n'
                "rem one-instance guard: drop any earlier bot container so two do not\r\n"
                "rem connect to the same Discord token (doubles every reply)\r\n"
                'for /f "tokens=*" %%i in (\'docker ps -aq --filter "name=automatch-bot-run"\') do docker rm -f %%i >nul 2>&1\r\n'
        ...
        cli.write_text(
            "#!/bin/sh\n"
            f'cd "{HERE}" || exit 1\n'
            "# one-instance guard: drop any earlier bot container first, so two do\n"
            "# not connect to the same Discord token (which makes the bot answer\n"
            "# every command twice and deadlock on its own run lock)\n"
            'ids=$(docker ps -aq --filter "name=automatch-bot-run"); '
