...
└── project_files/
    ...
    ├── LICENSE                     MIT license
    ├── automatch/                  the application itself (runs inside the container)
    │   ├── __main__.py             makes `python -m automatch` run the app
    ...
    └── docs/
        ...
        ├── resume_template.example.docx   the same resume with a <tag> in the skills line (advanced)
