"""automatch CLI.
  ...
  python -m core run            scrape -> score -> rank
  python -m core run -r 5       quick test (5 results per search term;
                                -r only affects the scrape step)
  python -m core score-only     score unscored jobs, then rank
  python -m core bot            the Discord resume-builder (advanced)
  python -m core rank-only      re-rank what's already scored (instant;
