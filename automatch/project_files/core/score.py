...
PROMPT_V = "v4-0to100-scale"  # bump when the judging method itself changes
...
def vector_prompt(vectors: dict, title: str, company: str, desc: str,
    ...
    lines = ["You are scoring a job posting for a specific candidate. "
             ...
             "from the real tasks.\n"
             "THEN score each metric as a WHOLE NUMBER from 0 to 100 using "
             "ONLY its anchor examples as the scale (the anchors sit at 0, "
             "20, 40, 60, 80, 100), judging the day-to-day reality you just "
             "described, NOT the posting's vibe. BE GRANULAR: interpolate "
             "between anchors and avoid round multiples of 10 (prefer e.g. "
             "63, 71, 78 over 60, 70, 80) so similar jobs get distinct "
             "scores.\n"]
